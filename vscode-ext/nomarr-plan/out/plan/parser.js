...
exports.getPhaseSteps = getPhaseSteps;
 ...
 */
function convertToPlan(mplan) {
    const phases = mplan.phases.map(mp => ({
        number: mp.number,
        title: mp.title,
 ...
 */
function extractStepId(text, phaseNum, stepNum) {
 ...
 */
function getPhaseSteps(plan, phaseName) {
    if (phaseName) {
        // Match by title (partial match)
        const lower = phaseName.toLowerCase();
        return plan.phases.find(p => p.title.toLowerCase().includes(lower) ||

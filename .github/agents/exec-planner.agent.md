...
1. **Research** — Use `read_module_api`, `locate_module_symbol` to understand existing code
2. **Identify scope** — What files will be created/modified
3. **Define phases** — Group related work (persistence, workflows, etc.)
4. **Define steps** — Actionable, verifiable steps within each phase
5. **Document contracts** — Methods this plan creates, methods it calls
6. **Write plan file** — Valid markdown per `task-plans.instructions.md`
7. **Update CONTRACTS.md** — Add new method signatures
8. **Update README.md** — Add plan to dependency graph if needed

### For AMEND

1. **Read existing plan** — Understand current structure
2. **Read review report** — What was missing
3. **Add new phase or steps** — Insert at appropriate point
4. **Update contracts** — New methods if any
5. **Preserve annotations** — Don't lose completed step notes
...
artifacts:
  - path: "plans/TASK-{feature}-{letter}-{title}.md"
    action: created | modified
  - path: "plans/dev/{feature}-parts/CONTRACTS.md"
    action: modified
  - path: "plans/dev/{feature}-parts/README.md"

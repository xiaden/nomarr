...
| Shape | Workflow |
|-------|----------|
| "What could we build?" | Ideator → Architect → Estimator → DDAuthor |
| "Design this feature" | Ideator → Architect → DDAuthor |
| "Here's a rough idea, flesh it out" | Ideator → Improver (refine) → Architect → DDAuthor |
...
**Default: Start with Ideator.** Unless the request is a pure estimation, pattern scan, or trivial fact check, Ideator explores the solution space first. Even "design this feature" benefits from option generation before committing to an approach.
...
1. **Run agents in dependency order** — Ideator before Architect (Architect needs options to analyze)
2. **Pass prior agent output as context** — each agent builds on the previous
3. **Use Improver to refine** — after any stage, Improver can iterate on the output
4. **Synthesize across reports** — You combine findings into a coherent recommendation
5. **Present to user or Director** — Summary + recommendation + supporting artifacts

...
def _validate_annotation(annotation: tuple[str, str]) -> dict[str, str] | None:
    """Validate annotation tuple. Returns error dict or None if valid."""
    marker, ann_text = annotation
    if not _MARKER_PATTERN.match(marker):
        return {
            "error": "invalid_annotation_marker",
            "message": f"Marker '{marker}' must be a single alphanumeric word (pattern: [A-Za-z0-9]+)",
        }
...
def _build_response(ctx: _ResponseContext) -> dict[str, Any]:
    ...
    if ctx.annotation_written and ctx.annotation:
        result["applied_annotation"] = {"marker": ctx.annotation[0], "text": ctx.annotation[1]}

    new_next = ctx.updated_dict.get("next")
    result["next_step"] = new_next if new_next else None
...
def complete_step(
    plan_name: str, step_id: str, workspace_root: Path, annotation: tuple[str, str] | None = None,
) -> dict[str, Any]:
    ...
    Args:
        ...
        annotation: Optional (marker, text) tuple. Marker must be a single
    ...
    updated_markdown, was_already_complete, annotation_written = mark_step_complete(plan, step_id, annotation)

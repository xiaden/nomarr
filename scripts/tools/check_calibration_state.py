...
ver = db.meta.get('calibration_version')
last_run = db.meta.get('calibration_last_run')
print(f'calibration_version:    {ver}')
print(f'calibration_last_run:   {last_run}')

# Print calibration_state summary
states = list(db.conn.aql.execute('FOR c IN calibration_state RETURN {label: c.label, p5: c.p5, p95: c.p95, n: c.sample_count}'))
...
sample = list(db.conn.aql.execute(
    'FOR f IN library_files FILTER f.calibration_hash != null LIMIT 1 RETURN f'
))
if sample:
    f = sample[0]
    fid = f['_id']
    ...
    mood_tags = list(db.conn.aql.execute(
    ...
    stats = list(db.conn.aql.execute(
...
sample2 = list(db.conn.aql.execute(
...
if sample2:
    ...
    mood_tags2 = list(db.conn.aql.execute(
        'FOR e IN song_has_tags FILTER e._from == @fid '
        'LET t = DOCUMENT(e._to) '
        'FILTER STARTS_WITH(t.rel, "nom:mood-") '
        'RETURN {rel: t.rel, val: t.value}',

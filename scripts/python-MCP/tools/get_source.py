...
Usage:
    # Standalone
    python scripts/mcp/tools/get_source.py myapp.database.Database.connect

    # As module
    from scripts.mcp.tools.get_source import get_source
    result = get_source("myapp.database.Database.connect")
"""
...
def get_source(qualified_name: str, *, context_lines: int = 0, max_lines: int | None = None) -> dict[str, Any]:
    ...
    Args:
        qualified_name: Fully qualified name with dots
            Examples:
            - "myapp.database.Database" (class)
            - "myapp.database.Database.connect" (method)
            - "myapp.utils.time.now_ms" (function)
...
def main() -> int:
    ...
    parser.add_argument("name", help="Qualified name (e.g., myapp.database.Database.connect)")
    parser.add_argument(

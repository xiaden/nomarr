...
Usage:
    # Standalone
    python scripts/mcp/tools/trace_calls.py myapp.api.routes.process_request

    # As module
    from scripts.mcp.tools.trace_calls import trace_calls
    result = trace_calls("myapp.api.routes.process_request")
"""
...
def _extract_imports(tree: ast.Module) -> dict[str, str]:
    ...
    E.g., {"db_service": "myapp.services.database"}
    """
...
def _extract_param_types(
    ...
    E.g., {"db_service": "myapp.services.database.DatabaseService"}

    Handles:
    1. Direct type annotations: `service: DatabaseService`
...
def _resolve_call(
    ...
    if parts[0] in param_types and len(parts) >= 2:
        ...
        # Type name format: myapp.module.path.ClassName
...
def _is_project_call(resolved: str | None, project_root: Path) -> bool:
    """Check if a resolved call is within the project (not stdlib or third-party).

    Args:
        resolved: Fully qualified name like "myapp.services.config_svc.ConfigService.load"
        project_root: Project root directory

    Returns:
        True if the call resolves to a file under project_root

    """
    if resolved is None:
        return False

    # Try to find the module file by traversing the dotted path
    parts = resolved.split(".")

    # Try progressively shorter module paths (from most specific to least)
    for i in range(len(parts), 0, -1):
        module_path = "/".join(parts[:i])

        # Check as a Python file
        potential_file = project_root / f"{module_path}.py"
        if potential_file.exists():
            return True

        # Check as a package directory
        potential_dir = project_root / module_path
...
def _trace_calls_recursive(qualified_name: str, project_root: Path, visited: set[str], depth: int) -> CallInfo | None:
    ...
    # Format: myapp.module.path.function or myapp.module.path.Class.method
    ...
    for call_expr, _call_line in raw_calls:
        ...
        # Skip non-project calls (stdlib/third-party) and duplicates
        if not _is_project_call(resolved, project_root):
            continue
...
def trace_calls(qualified_name: str, project_root: Path | None = None) -> dict[str, Any]:
    ...
    Args:
        qualified_name: Fully qualified function name
            Examples:
            - "myapp.api.routes.handle_request"
            - "myapp.services.RequestService.process"

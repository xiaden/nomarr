...
def main() -> int:
    ...
    Derives layer roots from pyproject.toml import-linter contracts.
    """
    import tomllib
    ...
    # Extract unique layer roots from import-linter contracts
    ...
    for contract in contracts:
        for key in ("source_modules", "forbidden_modules"):
            modules = contract.get(key, [])
            if isinstance(modules, str):
                modules = [modules]
            for mod in modules:
                # Only top-level nomarr.* modules (e.g., nomarr.services, not nomarr.persistence.database)
                parts = mod.split(".")
                if len(parts) >= 2 and parts[0] == "nomarr":
                    layer_roots.add(f"{parts[0]}.{parts[1]}")

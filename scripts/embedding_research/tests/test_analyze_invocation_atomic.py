"""Spec-first tests for the obligation-backed analyze invocation terminal state (Plan B P1).

Research-only.  Pins the execution-reporting "atomic analyze" contract (CONTRACTS.md): an analyze
invocation is ``running`` (its obligations record present) until every requested backbone, every
representation-class comparability decision, and every MANDATORY ``global_pool:{backbone}:medoid``
baseline resolves; views/scopes are append-only EVIDENCE and never terminalize the invocation; only
exactly one clean terminal ``completed`` outcome is reportable; any later failure (after an earlier
class or on a later backbone) makes the invocation ``failed`` and excludes it from completed-report
selection.
...
def _seed_baseline_evidence(con, run_id: str, backbone: str) -> str:
    """Persist MANDATORY ``global_pool:{backbone}:medoid`` baseline rows (the obligation evidence)."""
    key = medoid_strategy_key_for(backbone)
    for k, val in ((5, 0.31), (10, 0.30)):
        con.execute(
            "INSERT INTO analyze_metrics "
            "(run_id, strategy_key, strategy_type, sim_metric, k, metric, value) "
            "VALUES (?, ?, ?, ?, ?, ?, ?)",
            (run_id, key, "global_pool", "cosine", k, "map", val),
        )
    return key
...
def _count_metrics(con, run_id: str) -> int:
    return con.execute("SELECT count(*) FROM analyze_metrics WHERE run_id=?", (run_id,)).fetchone()[0]
...
def test_failure_after_first_class_leaves_open_invocation_and_partial_evidence(con) -> None:
    ...
    rid = "r-fail1class"
    record_analyze_invocation(con, run_id=rid, backbones=[("effnet", medoid_strategy_key_for("effnet"))])
    ...
    assert _count_metrics(con, rid) > 0
...
def test_second_backbone_failure_leaves_open_invocation_fail_closed(con) -> None:
    ...
    record_analyze_invocation(
        ...
        run_id=rid,
        backbones=[
            ("effnet", medoid_strategy_key_for("effnet")),
            ("musicnn", medoid_strategy_key_for("musicnn")),
        ],
    )
    ...
    assert _count_metrics(con, rid) > 0  # partial evidence retained
    assert rid not in run_mod._completed_analyze_run_ids(con)
...
def test_complete_plus_failed_rows_veto_run_and_refuse_report(con) -> None:
    ...
    # run-scoped metrics exist but no completed scope -> the report phase fails closed.
    assert _count_metrics(con, rid) > 0
...
def test_only_clean_obligation_run_resolves_amid_failed_sibling(con) -> None:
    ...
    record_analyze_invocation(con, run_id=clean, backbones=[("effnet", medoid_strategy_key_for("effnet"))])
    ...
    record_analyze_invocation(con, run_id=bad, backbones=[("musicnn", medoid_strategy_key_for("musicnn"))])

"""Filesystem cache for per-bin mean CTP head activations.

CTP (Classify-Then-Pool) segmentation drives bin boundaries from a head's
score-stream STD-binning.  This module stores the mean head-activation vector
for each bin so that downstream analysis can compute weighted song scores
...
CACHE_BASE: Path = _OUTPUT_ROOT / "cache" / "binned_ctp_heads" / _cache_semantics_tag()


def _purge_corrupt(p: Path) -> None:
    try:
        p.unlink()
        _log.warning("Deleted corrupt CTP-heads cache file (will recompute): %s", p)
    except OSError as e:
        _log.warning("Could not delete corrupt CTP-heads cache file %s: %s", p, e)

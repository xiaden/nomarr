...
SIM_CACHE_BASE: Path = _OUTPUT_ROOT / "cache" / "sim" / _cache_semantics_tag()

_AGG_METHODS: tuple[str, ...] = tuple(AGG_METHODS)
...
def sim_cache_path(
    ...
    return SIM_CACHE_BASE / backbone / bin_mode / _threshold_key(std_thresh) / fname
...
def load_sim(
    ...
    try:
        ...
        mats: dict[str, np.ndarray] = {agg: data[f"sim_{agg}"] for agg in _AGG_METHODS if f"sim_{agg}" in data.files}
        if not mats:
            _log.warning(

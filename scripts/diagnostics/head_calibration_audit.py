...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        "--password", default=None, help="Defaults to docker/.env ARANGO_ROOT_PASSWORD, then nomarr_dev_password"
    )
    return p.parse_args()
...
def main() -> None:
    ...
    calib_map: dict[tuple[str, str], dict] = {(c["head"], c["label"]): c for c in calib_rows}
    ...
    for tr in tier_rows:
        key = tr["value"]
        if tr["rel"] == "nom:mood-strict":
            tier_hits[key]["strict"] = tr["n"]
        elif tr["rel"] == "nom:mood-regular":
            tier_hits[key]["regular"] = tr["n"]
        elif tr["rel"] == "nom:mood-loose":
            tier_hits[key]["loose"] = tr["n"]
    ...
    sweep_header = f"  {'HEAD / LABEL':<48}" + "".join(f"  @{t:.2f}" for t in SWEEP_THRESHOLDS)
    ...
    if exp_sweep_data:
        exp_header = f"  {'HEAD / LABEL':<48}" + "".join(f"  ^{e:.2f}" for e in SWEEP_EXPONENTS)

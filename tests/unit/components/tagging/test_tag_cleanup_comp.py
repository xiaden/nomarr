...
def _make_cleanup_db(
    *,
    ...
    mock_db.tags._id.collect.return_value = all_tag_ids
    mock_db.song_has_tags.count.return_value = len(song_edge_targets)
    mock_db.song_has_tags._to.collect.return_value = song_edge_targets
    mock_db.tag_model_output.count.return_value = len(model_edge_sources)
    mock_db.tag_model_output._from.collect.return_value = model_edge_sources
    mock_db.tags.cascade.return_value = cascade_result
    return mock_db
...
class TestCleanupOrphanedTags:
    ...
    def test_returns_zero_and_skips_cascade_when_no_tags_exist(self) -> None:
        ...
        assert result == 0
        mock_db.tags.cascade.assert_not_called()
    ...
    def test_cascades_orphan_ids_and_returns_deleted_count(self) -> None:
        ...
        assert result == 2
        mock_db.tags.cascade.assert_called_once()
        cascaded_ids = mock_db.tags.cascade.call_args.args[0]
        assert set(cascaded_ids) == {"tags/3", "tags/4"}

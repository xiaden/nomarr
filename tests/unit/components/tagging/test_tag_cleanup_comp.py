...
class TestCleanupOrphanedTags:
    ...
    @pytest.mark.unit
    @pytest.mark.mocked
    async def test_returns_zero_and_skips_delete_when_no_orphans(self) -> None:
        mock_db = _make_cleanup_db(orphan_ids=[])

        result = await cleanup_orphaned_tags(mock_db)
    ...
    @pytest.mark.unit
    @pytest.mark.mocked
    async def test_deletes_orphans_and_returns_deleted_count(self) -> None:
    ...
    @pytest.mark.unit
    @pytest.mark.mocked
    async def test_delegates_to_library_facade(self) -> None:
        mock_db = _make_cleanup_db(orphan_ids=["tags/1"], delete_result=1)

        await cleanup_orphaned_tags(mock_db)
...
class TestGetOrphanedTagCount:
    ...
    async def test_returns_zero_when_no_orphans(self) -> None:
        ...
        result = await get_orphaned_tag_count(mock_db)

        assert result == 0

    @pytest.mark.unit
    @pytest.mark.mocked
    async def test_returns_count_of_orphaned_tags(self) -> None:
    ...
    async def test_delegates_to_library_facade(self) -> None:
        ...
        await get_orphaned_tag_count(mock_db)

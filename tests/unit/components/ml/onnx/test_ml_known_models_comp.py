...
class TestKnownModelsLabelOrdering:
    ...
    # -- engagement (upstream model: idx0=low-engagement/atmospheric, idx1=captivating)

    def test_engagement_2c_index0_is_atmospheric(self) -> None:
        """Index 0 of engagement_2c is the *low-engagement* (atmospheric) class."""
        assert self._label_at("engagement_2c-discogs-effnet-1", 0) == "Atmospheric"

    def test_engagement_2c_index1_is_captivating(self) -> None:
        """Index 1 of engagement_2c is the *high-engagement* (captivating) class."""
        assert self._label_at("engagement_2c-discogs-effnet-1", 1) == "Captivating"
...
class TestBuildOpponentMap:
    ...
    def test_atmospheric_and_captivating_are_opponents(self) -> None:
        assert "Captivating" in OPPONENT_MAP.get("Atmospheric", set())
        assert "Atmospheric" in OPPONENT_MAP.get("Captivating", set())

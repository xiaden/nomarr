...
def test_get_files_by_ids_with_tags_hydrates_tags_and_library_ids() -> None:
    ...
    db.library.list_files_by_ids.return_value = [
        {"_id": "library_files/1", "path": "D:/Music/song.flac", "library_key": "1"}
    ]
...
def _obsolete_search_files_by_tag_numeric_sorts_by_distance_and_hydrates_tags() -> None:
    ...
    db.library.list_files_by_ids.return_value = [
        {"_id": "library_files/1", "artist": "B", "album": "A", "title": "Far", "library_key": "1"},
        {"_id": "library_files/2", "artist": "A", "album": "A", "title": "Near", "library_key": "1"},
    ]

    db.library.list_file_tags_for_files.return_value = {
        "library_files/1": [{"name": "nom:bpm", "value": 118.0}],
        "library_files/2": [{"name": "nom:bpm", "value": 121.0}],
    }
    ...
    db.library.list_files_by_ids.assert_called_once_with(["library_files/2"])

    db.library.list_file_tags_for_files.assert_called_once_with(["library_files/2"])
...
def test_search_files_by_tag_numeric_sorts_by_distance_and_hydrates_tags() -> None:
    ...
    db.library.list_tags_by_name.assert_called_once_with("nom:bpm", limit=2)
    db.library.get_song_tag_edges_for_tags.assert_called_once_with(["tags/1", "tags/2"], limit=DEFAULT_LIMIT)
    db.library.list_files_by_ids.assert_called_once_with(["library_files/1", "library_files/2"])
    db.library.list_file_tags_for_files.assert_called_once_with(["library_files/2"])
...
def test_require_library_file_id_returns_id_for_existing_file() -> None:
    ...
    with patch("nomarr.components.library.library_file_query_comp.get_library_file") as mock_get_library_file:
        mock_get_library_file.return_value = {"_id": "library_files/123"}
...
def test_list_all_file_ids_filters_non_string_ids_and_uses_default_limit() -> None:
    ...
    assert result == ["library_files/1", "library_files/4"]
...
def test_find_move_candidate_by_chromaprint_normalizes_library_id() -> None:
    ...
    candidate = {"_id": "library_files/9", "chromaprint": "abc123"}

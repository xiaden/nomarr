...
class TestCalibrationRepo:
    ...
    def test_record_history(self, pg_session) -> None:
        """record_history should insert and return a history record."""
        _insert_model(pg_session, "hist_model")
        repo = CalibrationRepo(pg_session)
        record = repo.record_history("hist_model", "calibration_started", {"iterations": 100})
        assert record["id"] > 0
    ...
    def test_get_history(self, pg_session) -> None:
        """get_history should return history records ordered by created_at desc."""
        _insert_model(pg_session, "gh_model")
        repo = CalibrationRepo(pg_session)
        repo.record_history("gh_model", "event_1", {"seq": 1})
        repo.record_history("gh_model", "event_2", {"seq": 2})

        results = repo.get_history("gh_model")
    ...
    def test_delete_state(self, pg_session) -> None:
        ...
        repo = CalibrationRepo(pg_session)
        record = repo.set_state("ds_model", {"x": 1})
        repo.delete_state(record["id"])

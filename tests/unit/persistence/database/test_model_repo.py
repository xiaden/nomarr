...
class TestModelRepo:
    ...
    def test_upsert_model_insert(self, pg_session) -> None:
    ...
    def test_get_model_existing(self, pg_session) -> None:
        ...
        result = repo.get_model("model_get")
        assert result is not None
        assert result["id"] == "model_get"
        assert result["model_type"] == "genre"

    def test_get_model_nonexistent(self, pg_session) -> None:
        """get_model should return None for a missing id."""
        repo = ModelRepo(pg_session)
        result = repo.get_model("nonexistent_model")
        assert result is None

    def test_get_model_by_type_existing(self, pg_session) -> None:
        ...
        result = repo.get_model_by_type("unique_type")
        assert result is not None
        assert result["id"] == "model_path"
        assert result["model_type"] == "unique_type"

    def test_get_model_by_type_nonexistent(self, pg_session) -> None:
    ...
    def test_update_model(self, pg_session) -> None:
        ...
        repo.update_model("model_upd", {"enabled": 0, "backbone_id": "bb_2"})
        result = repo.get_model("model_upd")
        assert result is not None
        assert result["enabled"] == 0
        assert result["backbone_id"] == "bb_2"
        assert result["model_type"] == "genre"  # unchanged

    def test_update_model_not_found(self, pg_session) -> None:
        """update_model should raise DatabaseStateError for a missing model."""
        repo = ModelRepo(pg_session)
        with pytest.raises(DatabaseStateError, match="not found"):
            repo.update_model("missing_model", {"enabled": 0})

    def test_delete_model(self, pg_session) -> None:
        ...
        repo.delete_model("model_del")
        result = repo.get_model("model_del")
        assert result is None

    def test_list_models(self, pg_session) -> None:
        ...
        result = repo.list_models()
        ids = [r["id"] for r in result]
        assert "list_1" in ids
        assert "list_2" in ids

    def test_count_models(self, pg_session) -> None:
        ...
        count = repo.count_models()
        assert count >= 2

    def test_get_models_by_ids(self, pg_session) -> None:
        ...
        result = repo.get_models_by_ids(["by_id_1", "by_id_3"])
        ids = [r["id"] for r in result]
        assert "by_id_1" in ids
        assert "by_id_3" in ids
        assert "by_id_2" not in ids

    def test_get_models_by_ids_empty(self, pg_session) -> None:
        """get_models_by_ids should return empty list for empty input."""
        repo = ModelRepo(pg_session)
        result = repo.get_models_by_ids([])
        assert result == []

    def test_get_enabled_models(self, pg_session) -> None:
        ...
        result = repo.get_enabled_models()
        ids = [r["id"] for r in result]
        assert "enabled_1" in ids
        assert "disabled_1" not in ids

    def test_get_by_backbone(self, pg_session) -> None:
        ...
        result = repo.get_by_backbone("bb_target")
        ids = [r["id"] for r in result]

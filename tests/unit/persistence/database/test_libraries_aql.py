...
class TestLibrariesAqlOperations:
    def test_insert_library_returns_id(self) -> None:
        ...
        with patch("nomarr.persistence.database.libraries_aql.insert_document", return_value="libraries/1"):
    ...
    def test_get_library_by_id_returns_dict_or_none(self) -> None:
        ops = LibrariesAqlOperations(MagicMock())
        with patch("nomarr.persistence.database.libraries_aql.get_many_by_field", return_value=[{"_id": "libraries/1"}]):
            assert ops.get_library_by_id("libraries/1") == {"_id": "libraries/1"}
        with patch("nomarr.persistence.database.libraries_aql.get_many_by_field", return_value=[]):
            assert ops.get_library_by_id("libraries/1") is None

    def test_get_library_by_id_propagates_execute_errors(self) -> None:
        ops = LibrariesAqlOperations(MagicMock())
        with (
            patch("nomarr.persistence.database.libraries_aql.get_many_by_field", side_effect=RuntimeError("db error")),
    ...
    def test_get_library_by_key_returns_dict_or_none(self) -> None:
        ops = LibrariesAqlOperations(MagicMock())
        with patch("nomarr.persistence.database.libraries_aql.get_many_by_field", return_value=[{"_key": "1"}]):
    ...
    def test_get_library_by_name_returns_dict_or_none(self) -> None:
        ops = LibrariesAqlOperations(MagicMock())
        with patch("nomarr.persistence.database.libraries_aql.get_many_by_field", return_value=[{"name": "Main"}]):
    ...
    def test_list_libraries_filters_non_dict_rows(self) -> None:
        ops = LibrariesAqlOperations(MagicMock())
        with patch("nomarr.persistence.database.libraries_aql.get_filtered_docs", return_value=[{"_id": "libraries/1"}, 1]):
            assert ops.list_libraries(enabled_only=True) == [{"_id": "libraries/1"}]

    def test_list_library_keys_filters_non_string_rows(self) -> None:
        ops = LibrariesAqlOperations(MagicMock())
        with patch("nomarr.persistence.database.libraries_aql.list_field_values", return_value=["main", 1]):
            assert ops.list_library_keys() == ["main"]

    def test_update_library_by_id_executes_update(self) -> None:
        ops = LibrariesAqlOperations(MagicMock())
        with patch("nomarr.persistence.database.libraries_aql.update_document_by_key") as update_mock:
            ops.update_library_by_id("libraries/main", {"watch_mode": "poll"})

        assert update_mock.call_args.args[1:] == ("libraries", "main", {"watch_mode": "poll"})

...
class TestDatabaseClearLibraryData:
    ...
    def test_clear_library_data_success(self, test_db):
        ...
        # Act
        test_db.library_files.clear_library_data()
...
class TestDatabaseClearOldJobs:
    ...
    def test_clear_old_jobs_success(self, test_db):
        ...
        # Act
        test_db.tag_queue.clear_old_jobs()
...
class TestDatabaseDeleteLibraryFile:
    ...
    def test_delete_library_file_success(self, test_db, temp_audio_file):
        ...
        # Act
        test_db.library_files.delete_library_file(path=str(temp_audio_file))
...
class TestDatabaseEnqueue:
    ...
    def test_enqueue_success(self, test_db, temp_audio_file, default_library):
        """Should successfully enqueue."""
        # Arrange
        library_path = LibraryPath(
            relative="test.mp3", absolute=Path(temp_audio_file), library_id=default_library, status="valid"
        )

        # Act
        result = test_db.tag_queue.enqueue(path=library_path)
    ...
    def test_enqueue_invalid_path_raises_error(self, test_db):
        ...
        # Arrange - create LibraryPath with not_found status
        library_path = LibraryPath(
            relative="nonexistent.mp3",
            absolute=Path("/nonexistent.mp3"),
            library_id=None,
            status="not_found",
        ...
        # Act & Assert
        # Enqueue validates path status - should reject non-valid paths
        # The enqueue function should check is_valid() and handle accordingly
        # For this test, we verify that invalid paths are properly constructed
        assert not library_path.is_valid()
        assert library_path.status == "not_found"
...
class TestDatabaseGetLibraryFile:
    ...
    def test_get_library_file_success(self, test_db, temp_audio_file, default_library):
        ...
        test_db.library_files.upsert_library_file(
            path=library_path, library_id=default_library, file_size=1024, modified_time=1234567890
        )
    ...
    def test_get_library_file_not_found(self, test_db):
        ...
        # Act
        result = test_db.library_files.get_library_file(path="/nonexistent/file.mp3")
...
class TestDatabaseGetLibraryScan:
    ...
    def test_get_library_scan_success(self, test_db):
        ...
        library_path = LibraryPath(
            relative="test.flac", absolute=Path("/music/test.flac"), library_id=None, status="valid"
        )
        job_id = test_db.library_queue.enqueue_scan(path=library_path, force=False)
        ...
        assert result["path"].replace("\\", "/") == "/music/test.flac"
    ...
    def test_get_library_scan_not_found(self, test_db):
        ...
        # Act
        result = test_db.library_queue.get_library_scan(scan_id=99999)
...
class TestDatabaseGetLibraryStats:
    ...
    def test_get_library_stats_success(self, test_db):
        ...
        # Act
        result = test_db.library_files.get_library_stats()
...
class TestDatabaseJobStatus:
    ...
    def test_job_status_success(self, test_db, temp_audio_file, default_library):
        """Should successfully job status."""
        # Arrange - enqueue job first
        library_path = LibraryPath(
            relative="test.mp3", absolute=Path(temp_audio_file), library_id=default_library, status="valid"
        )
        job_id = test_db.tag_queue.enqueue(path=library_path)
...
class TestDatabaseListLibraryFiles:
    ...
    def test_list_library_files_success(self, test_db):
        ...
        # Act
        test_db.library_files.list_library_files()
...
class TestDatabaseListLibraryScans:
    ...
    def test_list_library_scans_success(self, test_db):
        ...
        # Act
        result = test_db.library_queue.list_library_scans()
...
class TestDatabaseQueueDepth:
    ...
    def test_queue_depth_success(self, test_db):
        ...
        # Act
        result = test_db.tag_queue.queue_depth()
...
class TestDatabaseQueueStats:
    ...
    def test_queue_stats_success(self, test_db):
        ...
        # Act
        result = test_db.tag_queue.queue_stats()
...
class TestDatabaseResetRunningLibraryScans:
    ...
    def test_reset_running_library_scans_success(self, test_db):
        ...
        # Act
        result = test_db.library_queue.reset_running_library_scans()
...
class TestDatabaseResetRunningToPending:
    ...
    def test_reset_running_to_pending_success(self, test_db):
        ...
        # Act
        result = test_db.tag_queue.reset_running_to_pending()
...
class TestDatabaseUpdateJob:
    ...
    def test_update_job_success(self, test_db):
        ...
        # Act
        test_db.tag_queue.update_job(job_id=1, status="pending")

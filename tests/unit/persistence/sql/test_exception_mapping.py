...
Tests the async context manager in ``nomarr.persistence.sql.exceptions``
...
@pytest.mark.asyncio
async def test_no_result_found_maps_to_entity_not_found():
...
@pytest.mark.asyncio
async def test_integrity_error_unique_violation_maps_to_duplicate_entity():
    ...
    with pytest.raises(DuplicateEntityError, match="Duplicate entity"):
        async with map_persistence_exceptions():
            raise exc


@pytest.mark.asyncio
async def test_integrity_error_fk_violation_maps_to_referential_integrity():
    ...
    with pytest.raises(ReferentialIntegrityError, match="Referential integrity violation"):
        async with map_persistence_exceptions():
            raise exc


@pytest.mark.asyncio
async def test_operational_error_maps_to_database_state_error():
...
@pytest.mark.asyncio
async def test_integrity_error_unknown_pgcode_maps_to_database_state_error(caplog):
...
@pytest.mark.asyncio
async def test_integrity_error_no_pgcode_maps_to_database_state_error():
    ...
    with pytest.raises(DatabaseStateError, match="Database error"):
        async with map_persistence_exceptions():
            raise exc


@pytest.mark.asyncio
async def test_integrity_error_null_orig_maps_to_database_state_error():

...
def locate_module_symbol(symbol_name: str) -> dict[str, Any]:
    ...
    search_paths = get_python_search_paths(config, root)
    ...
    if not search_paths:
        return {
            ...
            "matches": [],
            "total_matches": 0,
            "error": "No Python search paths configured or detected",
        }

    # Detect top-level folders from search paths for disambiguation
    ...
    for search_path in search_paths:
        ...
        if search_path.is_dir():
            for item in search_path.iterdir():
                if item.is_dir():
                    top_level_folders.add(item.name)
    ...
    # Search all Python files in configured search paths
    ...
    if len(matches) > 5:
        ...
        return {
            ...
            "warning": (
                f"Symbol '{symbol_name}' is too common ({len(matches)} matches). "
                f"Showing file, line, and qualified name only. "
                f"Use a more specific query (e.g., 'services.{target_name}')."
            ),
        }
...
def _search_tree(
    ...
    # (e.g., "nomarr/services/config_svc.py" → "nomarr.services.config_svc")

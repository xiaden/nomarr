...
def list_project_directory_tree(folder: str = "", *, workspace_root: Path) -> dict[str, Any]:
        ...
        for item in items:
            ...
            if item.is_dir():
                ...
                if max_depth is not None and current_depth >= max_depth - 1:
                    ...
                    tree[item.name + "/"] = build_tree(
                        item, max_depth, current_depth + 1, show_files_at_root_only
                    )
            # Include files based on mode
            elif show_files_at_root_only:
                # Only show files at depth 0 (workspace root)
                if current_depth == 0 and item.suffix in {
                    ".md",
                    ".py",
                    ".json",
                    ".toml",
            ...
            elif current_depth == 0 or item.suffix in {
                ".md",
                ".py",
                ".json",
                ".toml",
    ...
    if folder == "":
        ...
        note = (
            "Complete folder structure. Files shown at root level only. "

...
def get_semantic_tool_examples() -> dict:
    ...
    try:
        ...
        try:
            result = read_module_api(module_name=module_name)
        ...
        try:
            result = locate_module_symbol(symbol_name="calculate_sum")
        ...
        try:
            result = read_module_source(qualified_name=f"{module_name}.calculate_sum")
            # Truncate source for display
            if "source" in result:
                result["source"] = (
                    result["source"][:100] + "..."
                    if len(result["source"]) > 100
                    else result["source"]
                )
        ...
        try:
            result = read_file_symbol_at_line(
                file_path=str(temp_path), line_number=3, workspace_root=temp_dir
            )
            # Truncate source for display
            if "source" in result:
                result["source"] = (

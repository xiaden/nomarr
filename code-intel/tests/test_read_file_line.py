...
def test_read_middle_line(tmp_path: Path) -> None:
    ...
    result = read_file_line(
        file_path="test.txt", line_number=5, workspace_root=tmp_path
    )
...
def test_read_first_line(tmp_path: Path) -> None:
    ...
    _write_file(tmp_path, "test.txt", SAMPLE_TEXT)
    result = read_file_line(
        file_path="test.txt", line_number=1, workspace_root=tmp_path
    )
    assert "error" not in result
...
def test_read_last_line(tmp_path: Path) -> None:
    ...
    _write_file(tmp_path, "test.txt", SAMPLE_TEXT)
    result = read_file_line(
        file_path="test.txt", line_number=10, workspace_root=tmp_path
    )
    assert "error" not in result
...
def test_read_second_line(tmp_path: Path) -> None:
    ...
    _write_file(tmp_path, "test.txt", SAMPLE_TEXT)
    result = read_file_line(
        file_path="test.txt", line_number=2, workspace_root=tmp_path
    )
    assert "error" not in result
    assert result["requested"]["start"] == 1
...
def test_line_out_of_range_high(tmp_path: Path) -> None:
    _write_file(tmp_path, "test.txt", SAMPLE_TEXT)
    result = read_file_line(
        file_path="test.txt", line_number=999, workspace_root=tmp_path
    )
...
def test_line_out_of_range_zero(tmp_path: Path) -> None:
    _write_file(tmp_path, "test.txt", SAMPLE_TEXT)
    result = read_file_line(
        file_path="test.txt", line_number=0, workspace_root=tmp_path
    )
    assert "error" in result
...
def test_file_not_found(tmp_path: Path) -> None:
    result = read_file_line(
        file_path="nonexistent.txt", line_number=1, workspace_root=tmp_path
    )
    assert "error" in result
...
def test_path_outside_workspace(tmp_path: Path) -> None:
    result = read_file_line(
        file_path="../../etc/passwd", line_number=1, workspace_root=tmp_path
    )
    assert "error" in result
...
def test_python_file_has_warning(tmp_path: Path) -> None:
    ...
    result = read_file_line(
        file_path="code.py", line_number=6, workspace_root=tmp_path
    )
...
def test_non_python_file_no_warning(tmp_path: Path) -> None:
    ...
    result = read_file_line(
        file_path="data.txt", line_number=5, workspace_root=tmp_path
    )
...
def test_include_imports_non_overlapping(tmp_path: Path) -> None:
    ...
    _write_file(tmp_path, "code.py", SAMPLE_PYTHON)
    result = read_file_line(
        file_path="code.py", line_number=9, workspace_root=tmp_path,
...
def test_include_imports_overlapping(tmp_path: Path) -> None:
    ...
    _write_file(tmp_path, "code.py", SAMPLE_PYTHON)
    result = read_file_line(
        file_path="code.py", line_number=3, workspace_root=tmp_path,
        include_imports=True,
    )
    assert "error" not in result
...
def test_relative_path(tmp_path: Path) -> None:
    ...
    result = read_file_line(
        file_path="sub/file.txt", line_number=1, workspace_root=tmp_path
    )
    assert "error" not in result
...
def test_absolute_path(tmp_path: Path) -> None:
    ...
    result = read_file_line(
        file_path=str(f), line_number=1, workspace_root=tmp_path
    )
    assert "error" not in result

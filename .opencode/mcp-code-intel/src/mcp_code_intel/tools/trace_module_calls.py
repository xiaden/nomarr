...
Usage:
    # Standalone
    python -m mcp_code_intel.trace_calls mypackage.module.function_name

    # As module
    from .trace_calls import trace_calls
    result = trace_calls("mypackage.module.function_name")
"""
...
def _extract_param_types(
    ...
    3. Depends with Any type: `service: Any = Depends(get_library_service)`
       - resolves via return annotation
    """
...
def _is_internal_call(resolved: str | None, include_patterns: list[str] | None = None) -> bool:
    ...
    Args:
        ...
        include_patterns: List of patterns like ["nomarr.*", "myapp.*"]
                         If None or empty, matches all non-stdlib calls

    Returns:
        True if resolved matches any pattern (or if no patterns configured)

    """
    if resolved is None:
        return False

    # If no patterns configured, include all calls (filter stdlib later if needed)
    if not include_patterns:
        # Exclude obvious stdlib/third-party
        stdlib_prefixes = ("builtins.", "typing.", "collections.", "functools.", "itertools.")
        return not any(resolved.startswith(p) for p in stdlib_prefixes)
...
def _trace_calls_recursive(
    ...
    for call_expr, _call_line in raw_calls:
        ...
        if not _is_internal_call(resolved, include_patterns):
            continue

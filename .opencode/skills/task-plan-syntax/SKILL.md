---
name: task-plan-syntax
description: Reference for task plan markdown syntax used by code-intel MCP tools. Covers required structure, format rules, step writing, splitting large tasks, and validation. Load when creating or editing files in artifacts/plans/.
---

# Task Plan Syntax Reference

Plans are parsed by `code-intel` MCP tools. Invalid structure causes parse errors.
...
Use code-intel MCP tools to work with plans:

- `plan_read(plan_name)` - Read and parse a plan file
- `plan_complete_step(plan_name, step_id, annotation_text)` - Mark step complete
- `plan_archive(plan_name)` - Archive completed plan

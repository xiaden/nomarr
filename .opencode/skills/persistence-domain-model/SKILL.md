...
- **Calibration facade leaks raw row shape (last ML surface to do so):** `get_calibration_state` / `get_calibration_state_view` / `list_calibration_states` (`ml.py:298-325`) return `CalibrationStateRecord` TypedDicts raw; `list_all_calibration_states_with_models` (`ml.py:512-515`) returns `list[dict[str, Any]]`. `replace_calibration_state` (`ml.py:517-527`) takes `payload: dict[str, Any]` raw.
...
- **state_data blob keys** (writer `ml_calibration_state_comp.py:94-112` `save_calibration_state`): `head_name`, `label`, `calibration_def_hash`, `histogram{lo,hi,bins,bin_width}`, `histogram_bins`, `p5`, `p95`, `n`, `underflow_count`, `overflow_count`, `updated_at`. The computed `key` (`head_name:label`, line 94) is stripped before persist (line 111) — NOT stored.
- **Flat-vs-nested state_data mismatch:** `get_histogram_for_head` (`calibration_svc.py:496-533`), `load_calibrations_from_db_wf` (`calibration_loader_wf.py:63-70`), `load_calibration_lookup` (`ml_calibration_state_comp.py:122-143`) read top-level `label`/`p5`/`p95`/`histogram_bins` from records nesting them under `state_data` → silently empty/None in prod. Test `test_calibration_svc.py:328-354` mocks a FLAT shape, so the mismatch is untested against the real nested shape. Loader/lookup/get_histogram_for_head paths are currently dead-coded (deadcode_allowlist:645,845-846) but the same trap applies to any future consumer.
- **LIVE wire mismatch:** `GET /calibration/histogram` (`calibration_if.py:141-151`) returns nested `{id, model_id, state_data, updated_at, backbone_id}` per item (`CalibrationRepo.list_states_with_models`, `calibration_repo.py:114-140`), but frontend `HeadHistogramResponse` (`frontend/src/shared/api/calibration.ts:130-143`) expects FLAT `model_key/head_name/label/histogram_bins/p5/p95/n/histogram_spec`.
- **Recommended dataclass:** `CalibrationState(model_id, head_name, label, calibration_def_hash, p5, p95, sample_count, underflow_count, overflow_count, histogram, histogram_bins|None, updated_at|None, backbone_id|None)` — drop int PK per `ModelOutput` precedent (`ml_model_output_dataclass.py:5-7`); mapper `calibration_state_from_record` flattens `state_data`; `calibration_state_payload` rebuilds the blob (key `n` ← `sample_count`). Delete-by-PK (`remove_calibration_state`, `ml.py:529`) stays persistence-internal; `delete_calibration_state` comp (`ml_calibration_state_comp.py:146-159`) must switch to identity-based delete.

...
**Last extended:** 2026-09-08 (song row-mirror leak scope, log L166)
...
## Song Row-Mirror Leak — Issue song-row-mirror-leaks-into-domain (2026-09-08, log L166)

Research scope for separating the songs-table row record from the application song contract (ADR-047 §5 enforcement). Governed by `artifacts/designs/parts/song-domain-repair/CONTRACTS.md` L298-315 (Corrected song intent boundary, 2026-08-30): **SongIdentity(library: LibraryIdentity(name, root_path), normalized_path) is domain identity**; `Song.from_row()`/`Song.to_dict()` are NOT facade/higher-layer APIs; row→domain mapping is persistence-owned. ADR-047 (2026-08-31) §5 bans row-shaped ctors/projections on domain classes; §8 leaves canonical Song lifecycle identity OPEN (pending this decision); §2 requires explicit adoption of generated IDs (none exists for `songs.id`).

- **The violating class:** `nomarr/helpers/dataclasses/song_dataclass.py:29-107` `Song` — frozen/slotted, 18 fields mirroring `songs` columns verbatim (`song_id: int` = PG PK, plus `library_id`, `folder_id`, `path`, `normalized_path`, timestamps…). `to_dict()` (55-80) emits storage-shaped dict keyed `"id"` (docstring admits "transitional projection"). `from_row()` (82-107). `__all__` exports `Song`, `SongTagMatch`. `helpers/dataclasses/__init__.py:16` re-exports Song.
- **Producers (persistence only — must switch to a persistence mapper, not `Song.from_row`):** `persistence/api/library_songs.py` get_song:172, get_song_by_path:178, get_song_by_normalized_path:184, find_song_by_path_any_library:189, list_songs_by_ids:193, list_songs:221, find_library_song_by_chromaprint:241, list_songs_for_folder:560, list_tracks_for_matching:574 (all `Song.from_row(row)`); `persistence/api/application.py:148` songs_with_state; `persistence/mappers/song_tag_mapper.py:111` song_from_row + `:117` song_tag_match_from_row (delegate to `Song.from_row`). `repo_dto.SongRow:28-47` is the persistence-private row DTO.
- **Int-keyed facade reads still live (NOT yet identity-keyed):** `get_song(song_id: int)` etc. — TASK-library-domain-facades-A Phase 3 explicitly deferred these "song-plan ownership" items: get_song, list_songs_by_ids, get_library_ids_for_songs, count_recently_tagged, update_library_song_path/scan_metadata/modified_time, set_library_song_chromaprint, update_library_song_last_tagged_at, remove_song, hydrate_song/hydrate_songs_batch. Identity BRIDGES exist: `library_songs.py` resolve_song_identity/resolve_song_identities (~111-163, int→SongIdentity) — bridge pattern proven by song-intent-facade-correction-A P3/P8.
- **Consumers that dictify (`Song.to_dict()` in production code — all must migrate):** `components/library/library_song_query_comp.py` L31 (get_song_by_id → dict), L130 (_get_songs_by_ids), L142 (_get_all_library_songs), L277/L280 (get_library_song) +16 more; `library_song_state_comp.py` L75/L84/L230/L421; `tag_stats_comp.py` L31/L38; `tag_query_comp.get_tag_songs_with_metadata`; mood_analysis_comp broken `.get("id")` (P4-S5 classified: `Song.song_id` OK only for the approved analytics projection). Raw dict reads: query comp `file_doc["id"]` L260/579, `["path"]` L484/494/533/554, `["normalized_path"]` L313/508; `require_library_song_id` returns `library_file["id"]` int PK (:293) to write_calibrated_tags_wf.py:149; discovery_worker.py:410-424 `get_song_by_id` → `file_doc["path"]` → `process_file_workflow(file_id=str(song_id))`; file_write_comp.py:46 `get_song_by_id(db, file_id)`.
- **Intentional int boundaries (NOT leaks — keep or explicitly authorize):** wire `file_id` contracts (frontend files.ts, TrackPlayData.file_id, FileTagResponse, claim key, RecentFileItem.file_id per AR-SDR-7/API-contract); `HydrateSongInput(song_id, …)` sole documented hydration exception; claims/state int handles stay as bridge INPUTS, never tag-facade args.
- **Sibling status:** TASK-song-intent-facade-correction-A fully executed (45/45 [x], 8 phases) but still in `pending/`; library-domain-facades-A..G (pending) converted Library-scoped methods to natural-key but left song-only/id-scoped methods to the song plan (this issue). Tests to update: test_library_song_query_comp.py, test_library_song_state_comp.py, test_tag_stats_comp.py, test_tag_query_comp.py, test_mood_analysis_comp.py, test_discovery_worker_private_helpers.py, test_write_calibrated_tags_wf.py; add facade-domain-boundary tests per `tests/unit/persistence/api/test_facade_domain_boundary.py` spec-first pattern.

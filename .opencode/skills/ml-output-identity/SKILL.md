...
- `alembic/versions/003_canonical_ml_output_streams.py` — `ml_output_streams` gains `output_id` String(255) NOT NULL, `output_index`, `values` JSONB; drops model_id/status.
...
### Finding 3: In-flight fix (feat/develop-branch-migration, 2026-08-29 working tree)
Done in tree (uncommitted): migration 005 adds `output_id` NOT NULL UNIQUE (md5 backfill, empty-table doctrine); ORM adds column; `store_model_output` = select-then-upsert by `output_id` with a `legacy:` fallback hash; facade `get_model_output(str)` / `replace_model_output` (passes output_key+index+label+fully_labeled) / `remove_model_output(str)`; registry reads `output_doc.get("output_id")`; tests re-pin the contract (test_ml_db.py:580 asserts `output_id="legacy_key"` forwarded; test_output_repo uses str output ids).
...
3. `song_id=0` hardcoded at register_ml_models_wf.py:95,103 + ml_svc.py:148 → FK violation (no songs.id=0) → prepare_database_wf.py:89 startup abort. Need song_id nullable in migration 005 + ORM `Mapped[int | None]` + callers pass None.
...
10. Migration 005 backfill `md5('legacy:'||id)` ≠ registry sha256 — fine only while tables empty.
...
- alembic/versions/003 + 005 (working tree, 005 untracked)

...
**The web frontend MUST ONLY call `/api/web/*` endpoints.** The `/api/v1/*` endpoints use API key authentication which the frontend cannot authenticate against.
...
Run `lint_project_frontend()` after every frontend change. Zero errors from ESLint and TypeScript is the only acceptable state.

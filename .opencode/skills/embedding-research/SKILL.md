...
- **No MaskRecord / audio-mask code** exists under scripts/embedding_research (case-insensitive `mask` hits are boolean dataframe masks in report/_base.py:158+, report/_binned.py:755, similarity.py:357+; prose in tests/test_audit_forbidden_vocabulary.py:305). No head-side "legacy" semantics remain: `LEGACY_RUN_ID` and the legacy head_phase statuses/readers were DELETED in the analyze/head hard cut (execution-reporting-repair Plan C). `db/_schema.py`'s only remaining `legacy` token is the StaleSchemaError refusal-guard SQL literal (`run_id='legacy'` rows are refused, never read as active).

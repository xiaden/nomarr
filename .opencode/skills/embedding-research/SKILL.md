...
## Corrective-pass research map (2026-09-04, branch feat/develop-branch-migration HEAD 137b6a88)

Research feeding the corrective-pass DD (supersedes the frozen-observation DD's deferred-cleanup posture: the prior DD deliberately RETAINED legacy code through "Plan F"; the corrective pass mandates a HARD CUT with no compatibility paths).

**Active 8-phase module graph** (run.py CLI_PHASES L892-901; _LEGACY_PHASES L845-854 is the legacy machinery to DELETE): ingest=`strategy_meta.ingest`; embed=`common/embed.embed` (writes `patches/{sid}.{bb}[.vN].npy` via `streams/StreamStore.publish`); infer-heads=`common/infer_heads.infer_heads` (writes `heads/{sid}.{bb}.npz`, one float32 [T,dim] array per head, T==backbone patch_count, refuses mismatch); catalog=`catalog.build_segmentation_catalog` (per-patch seg_membership rows today — the corrective pass replaces with compact config×song×segment + sparse absorbed exceptions); catalog-report=`catalog_report`; analyze=`common/catalog_analysis.run_catalog_analysis` → `search_views.materialize_search_view` (always-regenerate view under `views/<keyset_hash>/`, gather via seg_meta.medoid_source_patch_idx) → `bounded_scoring.score_bounded_exact` → `db/analyze_scope.write_catalog_analyze_rows` (writes analyze_metrics + song_retrieval_metrics + run scope); head-analysis=`common/head_analysis.run_shared_ptc_head_pooling` (canonical CPU runner) + `db/head_phase.write_head_phase_provenance`; report=`report.run`.

**Legacy still present (delete targets):** run.py L501-825 old phase fns (_segment_phase/_classify_phase/_head_phase/_load_{global_pool,ptc,ctp}_analyze_vecs/ANALYZE_CFG dicts/ModelCache/_build_model_cache/_build_ctp_segment_infra/_reset_cache_dirs) + sole-use imports (cache.binned_ctp/binned_ptc/flat_vecs, cache_identity, common.analyze, classify, head_pooling, strategy_*segment_fn, strategy_binned._constants). classify.py (run_flat/run_binned/run_ptc_heads + archival run_shared_ptc_head_pooling) and head_pooling.py (LEGACY-interim duplicate of common/head_analysis.py's 4 names) delete together. db/head_phase.py dual: canonical write_head_phase_provenance/build_head_phase_provenance_rows vs archival append_head_phase_archival_rows/build_archival_provenance_rows — keep only canonical. db/_schema.py still DDLs 24 tables incl. dead: pooled_vecs/head_results/binned_calibration/head_agreement_rows/binned_pair_sims/patch_features/binned_classify_ctp/binned_ctp_vecs/binned_ptc_ctp_metrics/head_sim_corr_rows/truncation_robustness_rows/binned_song_stats. cache/{flat_vecs,flat_heads,binned_ptc,binned_ptc_heads,binned_ctp,binned_ctp_heads}/ strategy_global_pool/ strategy_ptc/ strategy_ctp/ strategy_binned/ common/segment.py common/analyze.py are legacy (cache dir gone means db/{flat,queries,songs} legacy reader fns + common/stratify head-bin source + report legacy decoders must be rewired or deleted).

**Threshold contract already resolved in code (R2 done):** helpers/thresholds.py resolve_threshold: direct_l2 default, effective==configured exactly, no calibration; std_scaled explicit opt-in requiring calibration basis (legacy historical). helpers/binning.py temporal_segment is canonical (unit-vector spherical running centroid, strict `>`, OUTLIER_WINDOW=3, global_dist=plain L2 L70-76, perdim Chebyshev L79). helpers/segmentation.py authoritative_segmentation + MembershipSegment: medoid observed in-range member only, select_medoid_source_index mean-incl-self centrality smallest-index ties (L70-90); weight=member_count INCL. absorbed (seg_meta.weight) — corrective pass redefines weight=searchable mass excl. absorbed+silence; candidate_weights_from_catalog (common/catalog_analysis.py L183-200) is the exact weight feed to change.

**Silence mask does NOT exist** in scripts/embedding_research (verified by grep). No audio-sidecar, no mask column/sidecar, no geometry silence. Corrective pass adds frozen audio-derived patch-aligned mask; open conflict: 8-phase CLI frozen, derived phases forbid audio (run.py DERIVED_FORBIDDEN_TOKENS incl. discover_audio), so mask production must be inside embed (audio-loading phase) or a new ingestion-time artifact — DD must decide.

**Hashes today (catalog_identity.py):** song_signature L234 (song identity+ready streams+seg_meta+membership rows), search_view_hash L305 (strict corpus: context lines+per-song signatures+canonical configs+all ready streams), catalog_state_payload/catalog_fingerprint L342/L362 (manifest-only over 8 logical tables, non-self-referential). Corrective pass splits/replaces search_view_hash with exact_segmentation_hash (boundaries+absorbed+silence+medoids+semantics) vs search_representation_hash (stream identity+ordered searchable medoid indices+normalized weights+scoring semantics) — search_views.SearchViewKey (L236-300) + db/analyze_scope.record_analyze_run_scope currently keyed on corpus search_view_hash need rework.

**Duplication/db note:** research.duckdb (DB_PATH) is the ONLY DuckDB today — catalog lives in it as tables, registry in it, disposable views as FS views/. No FS-first manifests, no catalogs/<id>.duckdb. reset preserves patches/ .npy sidecars only. Stream/HeadStream records carry full provenance fields in registry rows (records.py StreamRecord L275 / HeadStreamRecord L364) — those rows are the natural manifest source when DB becomes disposable index.

...
`.pre-commit-config.yaml`; `readme.md:158-160` and `CONTRIBUTING.md:56` state
...
`lint_project_frontend`), uv-managed venv requirements, no-pre-commit status.
...
- **Location:** `.github/workflows/backend-quality.yml:48` (job `lint`; install `:69-70`, commands `:72-90`), `:92` (job `deptry`; install `:113-114`, command `:116-119`)
...
- **Location:** `.github/workflows/backend-tests.yml:58` (job `test`; install `:84-85`, run `:87-88`), `:97` (job `architecture-qc`; install `:118-119`, run `:121-122`), `:172` (job `database-tests`; install `:193-194`, run `:199-200`)
- **What:** Three jobs:
...
- **Location:** `pyproject.toml:93-126`
...
- **Location:** `pyproject.toml:127-154` (`[tool.mypy]`), `:155-249` (`[tool.ruff]`; sub-tables `[tool.ruff.lint]` `:167`, `[tool.ruff.lint.per-file-ignores]` `:199`, `[tool.ruff.format]` `:227`, `[tool.ruff.lint.isort]` `:233`, `[tool.ruff.lint.flake8-tidy-imports.banned-api]` `:236`)
- **What:** ruff `line-length = 120` (NOTE: CONTRIBUTING.md:278 says 100 — the formatter config is authoritative at 120), `output-format = "json"`, `fix = true`, `unsafe-fixes = true`; banned-api rules redirect `time.time`/`datetime.datetime.now`/`builtins.print` to `nomarr.helpers.time_helper`. mypy: `python_version = "3.12"`, `ignore_missing_imports = true`, excludes tests/scripts/artifacts.
...
- Backend requires Python 3.12 and a uv-managed `.venv` created by `uv sync --locked` (uv >= 0.12.13; `[tool.uv] default-groups = ["dev", "cpu"]` installs the complete toolchain — pytest, ruff, mypy, import-linter, deptry); frontend requires Node 24 (CI) / Node 18+ (CONTRIBUTING) with `npm ci`.

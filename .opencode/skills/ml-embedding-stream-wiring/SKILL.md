...
Nomarr's ML pipeline currently computes per-patch backbone embeddings for the whole waveform, then pools ALL patches into ONE track vector via trimmed_mean. Per ADR-038 (Accepted, 2026-06-20) + DD-int8-canonical-embedding-stream-storage (Completed, Arango-era), the per-patch stream was designed to be stored in `ml_embedding_streams` as the SOURCE OF TRUTH. The current restoration DD (`artifacts/designs/pending/DD-restore-quantized-embedding-streams.md`, 2026-08-30) reopens the quantization level and selects **uniform FP16 for both persisted representations**: FP16 bytes for the temporal stream and PostgreSQL `HALFVEC`/FP16 for the derived lookup, subject to benchmark gates. The `embeddings` table is a DERIVED lookup table (one pooled vector per (song, backbone)) recomputed from the stored stream when segmentation changes; segmentation is a read-time/post-hoc user-configurable operation. **The design was never wired into production in any era** — git history shows zero `embedding_stream` references in workflows/services ever, and `ml_embedding_stream_store_comp` was never created. Today the pipeline pools whole-waveform embeddings inline (non-segmented) and writes the `embeddings` table directly with `segmentation_hash=None` hardcoded — the stream is discarded.

## Coverage

**Documented:** Table/repo/facade contract (all validated, zero prod callers), broken direct-write path (file:line evidence), quantization design + validation **and its R7 amendment** (current research report: uniform FP16 stream and HALFVEC/FP16 derived lookup; alternative levels are not selected; missing validation script; gate benchmark mandatory), schema gaps vs DD data model, derive-time vs embedding-time segmentation options, hot/cold maintenance alignment, deferred-write DTO pattern.

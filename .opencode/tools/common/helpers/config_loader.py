...
def get_venv_binary(project_root: Path, name: str) -> Path:
    ...
    if sys.platform == "win32":
        candidates = [
            ...
            project_root / ".venv-linux" / "Scripts" / f"{name}.exe",
        ]
    else:
        candidates = [
...
def get_python_search_paths(config: dict | None = None, workspace_root: Path | None = None) -> list[Path]:
    ...
    if not search_paths:
        search_paths.extend(
            item for item in workspace_root.iterdir() if item.is_dir() and (item / "__init__.py").exists()
        )

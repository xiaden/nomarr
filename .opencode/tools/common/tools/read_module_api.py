...
def _module_to_path(module_name: str, workspace_root: Path | None = None) -> Path | None:
    ...
    from common.helpers.config_loader import get_python_search_paths, load_config
    ...
    for base in search_bases:
        ...
        module_path = base / "/".join(parts[:-1]) / f"{parts[-1]}.py" if len(parts) > 1 else base / f"{parts[0]}.py"
...
def _resolve_base_classes(
    ...
    for base in class_node.bases:
        ...
        # Prevent cycles
        class_path = f"{base_module}.{base_name}" if base_name and "." not in base_module else base_module
...
def _extract_imports(tree: ast.Module, current_module: str, *, is_package: bool = False) -> dict[str, str]:
    ...
    for node in tree.body:
        ...
        # Constants (uppercase names with simple value assignments)
            ...
            for target in node.targets:
                if isinstance(target, ast.Name) and target.id.isupper() and target.id not in EXCLUDED_CONSTANTS:
                    try:
...
if __name__ == "__main__":
    ...
    result = read_module_api(
        ...
        include_docstrings=raw.get("include_docstrings", True),
        include_inherited=raw.get("include_inherited", True),
    )

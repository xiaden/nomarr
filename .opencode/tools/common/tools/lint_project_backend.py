...
Runs ruff (check --fix --unsafe-fixes + format), mypy, import-linter, and pytest on specified path.
...
from common.helpers.config_loader import get_venv_binary, get_workspace_root, load_config
...
def lint_project_backend(path: str | None = None, check_all: bool = False) -> dict[str, Any]:
    ...
    if target_files and (DEBUG_LINTER is None or DEBUG_LINTER == "ruff"):
        venv_ruff = get_venv_binary(project_root, "ruff")
        try:
            result = subprocess.run(
                [
                    str(venv_ruff),
    ...
    if target_files and (DEBUG_LINTER is None or DEBUG_LINTER == "mypy"):
        venv_mypy = get_venv_binary(project_root, "mypy")
    ...
    if scope_includes_nomarr and (DEBUG_LINTER is None or DEBUG_LINTER == "import-linter"):
        venv_lint_imports = get_venv_binary(project_root, "lint-imports")
        try:
            result = subprocess.run(
    ...
    # Run pytest — full suite when check_all=True, scoped to related tests otherwise
    if DEBUG_LINTER is None:
        venv_python = get_venv_binary(project_root, "python")
            ...
            if check_all:
                ...
                for f in target_files:
                    ...
                    if stem.startswith("test_"):
                        # Changed file is itself a test file
                        found.add(f)
                    else:
                        # Look for matching test file(s) anywhere under tests/
                        for match in test_dir.rglob(f"test_{stem}.py"):
                            found.add(str(match))
                pytest_targets = sorted(found)

            if not pytest_targets:
                result_json["pytest"] = {"status": "skipped", "reason": "no related tests found"}
            else:
                try:
                    ...
                    # Parse counts — order varies e.g. "1 failed, 787 passed, 2 deselected in 6s"
                    combined_output = pytest_stdout + pytest_stderr
    ...
    # Add summary — when target is a directory, count actual .py files for accuracy
    if len(target_files) == 1 and Path(target_files[0]).is_dir():
        _exclude = {".venv", "__pycache__", ".git", "node_modules", ".mypy_cache", ".ruff_cache"}
        files_checked = sum(1 for p in Path(target_files[0]).rglob("*.py") if not _exclude.intersection(p.parts))
    else:
...
if __name__ == "__main__":
    ...
    result = lint_project_backend(
        path=raw.get("path"),
        check_all=raw.get("check_all", False),
    )

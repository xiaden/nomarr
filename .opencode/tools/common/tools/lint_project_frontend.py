...
def _npm_cmd(*args: str) -> list[str]:
    """Build an npm command that resolves correctly on all platforms."""
    if os.name == "nt":
...
def _npx_cmd(*args: str) -> list[str]:
    """Build an npx command that resolves correctly on all platforms."""
    if os.name == "nt":
        return ["cmd", "/d", "/c", "npx", *args]
    return ["npx", *args]


def _as_text(value: str | bytes | None) -> str:
    """Normalize subprocess text output values.

    Some subprocess invocations may yield None for stdout/stderr.
    """
    if value is None:
        return ""
...
def _combine_output(stdout: str | bytes | None, stderr: str | bytes | None) -> str:
    """Combine stdout/stderr into a safe, human-readable message."""
    combined = (_as_text(stdout) + "\n" + _as_text(stderr)).strip()
            ...
            all_errors.append(
                {
                    ...
                    "code": "test-failure",
                    "severity": "error",
                    "message": _combine_output(test_result.stdout, test_result.stderr)[-2000:],
                    "fix_available": False,
                },
            )
    except Exception as e:

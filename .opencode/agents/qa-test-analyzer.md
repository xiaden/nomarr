...
Use `read_module_source` to check current signatures when something looks off.
...
- **Is the test stale?** Check if it references renamed/removed methods or passes outdated arguments. Use `read_module_api` and `locate_module_symbol` to compare what the test expects vs what the implementation provides.

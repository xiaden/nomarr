...
## Key Modules
 ...
 | `sync_file_to_library_wf.py` | Canonical file sync — upserts `songs`, parses tags, seeds entity graph, rebuilds cache |
...
- **Move detection**: Both quick and full scans reconcile moves interleaved with the per-folder walk, not in a separate first pass. Within each successfully-walked folder, modified files upsert in place, same-locator root/mount rebases apply in place, and each genuine new file is resolved individually via a bounded, library-scoped chromaprint lookup before it is treated as a genuine insert. The lookup reports whether the candidate population was observed in full; a truncated window refuses automatic relocation rather than choosing from the deterministic first window. Genuine deletes happen only in a deferred scoped cleanup pass over successfully-walked or vanished folders; unchanged (cache-skipped) and failed folders are never reconciled or cleaned. A match is applied only when its source is genuinely absent from disk, so a live duplicate is never repointed. Quick scan reconciles only the folders it walked; full scan walks every folder.

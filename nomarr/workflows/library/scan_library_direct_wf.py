...
def scan_library_direct_workflow(
    ...
    Args:
        ...
        scan_path: Root path to scan (from library record)
    ...
    try:
        ...
        logger.info(f"[scan_library] Counting files in {scan_path}...")
        total_files = count_audio_files(scan_path, recursive=True)
        ...
        for _folder_path, files in walk_audio_files_batched(scan_path, recursive=True):

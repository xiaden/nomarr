...
def scan_library_quick_workflow(
    ...
    try:
        ...
        # folders are never touched. ``get_songs_for_folder`` is prefix-recursive,
        # so an eligible ancestor's lookup also surfaces rows in nested descendant
        # folders. Per row, reuse the candidate source-presence guard: delete only
        # rows whose own normalized parent folder was reconciled by this scan AND
        # whose current absolute path no longer exists. This keeps rows belonging
        # to a nested cache-skipped/failed subtree out of the cleanup scope (a
        # relocated row now points at its new, existing path and is not deleted).

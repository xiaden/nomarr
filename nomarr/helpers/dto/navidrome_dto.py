...
class TasteProfile(TypedDict):
    """Per-user taste profile computed from caller-provided play history.

    Play history arrives through the Navidrome plugin / API request boundary;
    Nomarr never reads it from local persistence.  Contains a list of
    genre-scoped clusters representing the user's multi-faceted listening
    preferences.  Each cluster has its own centroid, track count, and weight,
    enabling per-genre ANN search and proportional interleaving in playlist
    builders.  Generated by ``compute_taste_profile`` component.
    """

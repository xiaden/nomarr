...
class TrackPlayData(TypedDict):
    """Play data for a single track.

    Provided by the Navidrome plugin or read from legacy local storage.
    ``nd_id`` is the Navidrome track identifier (optional when data comes
    from the plugin, which resolves tracks to Nomarr ``file_id`` upstream).
...
class TasteProfile(TypedDict):
    ...
    Contains a list of genre-scoped clusters representing the user's
    multi-faceted listening preferences.  Each cluster has its own centroid,
    track count, and weight, enabling per-genre ANN search and proportional
    interleaving in playlist builders.  Generated by ``compute_taste_profile``
    component.
    """
...
class TasteCluster(TypedDict):
    ...
    ``total_weight`` is the sum of recency weights across all tracks in
    this cluster, used by playlist builders for proportional interleaving
    (clusters with higher weight receive more playlist slots).
    """
...
class NavidromePersonalPlaylistContext(TypedDict):
    ...
    drives proportional interleaving across genres.
...
class NavidromePersonalPlaylistEntry(TypedDict):
    ...
    track reference values — Navidrome ``nd_id`` resolution happens

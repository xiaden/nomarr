...
- Claim files atomically via the `db.app.add_claim` intent to prevent duplicate processing
...
- **Crash recovery**: Claim cleanup (`cleanup_stale_claims` / worker-death release) removes claims for stale workers and missing/completed/errored songs; files become re-discoverable

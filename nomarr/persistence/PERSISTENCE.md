...
**Mood ownership:** `LibraryTagsDb` is the sole semantic owner of the exact `SongIdentity`-addressed `replace_mood_tags` and `replace_mood_tags_batch` contracts, including the three literal `nom:mood-strict`, `nom:mood-regular`, and `nom:mood-loose` tiers, the private additive calibration marker, and same-transaction assignment/marker ownership. The Tier-2 `SongTagRepository` owns the private locator-addressed mood SQL, the single short transaction, commit/rollback, exception mapping, poisoned-connection disposal, and bounded fresh-connection retry; `LibraryTagsDb` only validates/canonicalizes the frozen commands and delegates the complete batch in one call. Batch validation rejects any repeated `SongIdentity` before any SQL: the whole batch returns `INVALID_VALUE` with no mutation, and duplicates are never folded/de-duplicated. These APIs do not permit generic mood replacement, integer identity, aliases/shims, or old/new dual paths. Historical resolver and integer-identity guidance remains confined to explicitly labeled provenance artifacts; current persistence ownership follows the locator-addressed boundary above.
...
**Identity boundary.** Persistence-private integer handles stay inside persistence. The locator-addressed application boundary is `SongIdentity`/`SongLocator`; public integer resolver bridges and ordinary integer `file_id`/`song_id` projections are forbidden and must not be reintroduced. Private repository joins may translate locators to storage rows internally, but no generated ID or row identity crosses the facade. Hydration is locator-addressed and payload-only: `HydrateSongInput` carries no `song_id`. Analytics and other semantic projections must use `SongIdentity`/`SongLocator` values or their opaque `nom1` wire tokens. `FileTag` may remain separately-owned physical-file metadata, but a projection such as `(file_id, tag_value)` is valid only when `file_id` is that opaque locator token—not a persistence integer.
...
**Sealed folder + library-scoped song facade** (`LibrarySongsDb` via `db.library.songs`): folder intents take a `Library` + a domain `LibraryFolder` and resolve `parent_path` → storage `parent_id` internally; `replace_library_folders` is path-stable so songs keep their folder linkage. Library-scoped song methods (`get_song_by_path`, `list_songs`, `count_songs_for_library`, `list_songs_for_folder`, …) take a `Library` and return domain `Song` values. Song-only operations are exposed through the current locator-addressed song intents; persistence keeps storage ids private.

**Library lifecycle admission and hydration serialization:** `PipelineRepository` serializes scan and tag-write admission on the parent library row, so a library cannot concurrently admit a scan and tag write. Tag-write admission also rejects any library with `not_hydrated` songs, and raises a typed `LibraryOperationConflict` containing the current axis states and hydration-debt count. Physical writes reject active scans. `LibrarySongsDb.replace_song_for_reimport` atomically removes and reinserts a locator-addressed snapshot, initializes the replacement's state assignments, and leaves fresh hydration/tag-write debt for later processing. `hydrate_song`, batch hydration, and `refresh_hydrated_song` perform their lifecycle guard inside the same repository unit of work as their mutations; hydration and refresh reject active tag writing rather than racing it. Hydration transitions `not_hydrated` to `hydrated`, while refresh updates file-derived projections without changing that state.
...
3. **Error mapping discipline** — SQLAlchemy exceptions raised by Tier 1 primitives are translated at the Tier 2 repository boundary via `map_persistence_exceptions()` in `sql/exceptions.py` into the domain exceptions in `nomarr/helpers/exceptions.py`: `EntityNotFoundError`, `DuplicateEntityError`, `ReferentialIntegrityError`, `DatabaseStateError`, and its specialized subclasses `RetryableDatabaseError` (retryable SQLSTATEs `40001`/`40P01`/`55P03`) and `AmbiguousCommitError` (commit completion unknown, e.g. `40003`). Raw SQLAlchemy exceptions propagate out of Tier 1 primitives and are only mapped at the Tier 2 repo boundary.

    **Ownership of SQL and transactions is tiered.** Tier 2 repository classes own table-scoped SQL, locator/storage-key resolution, one short unit-of-work transaction, commit/rollback, poisoned-session disposal, and bounded fresh-session retry (for example `SongTagRepository.replace_mood_tags_batch` composes the UoW-safe `TagRepository.get_or_create_tags_batch` primitive and owns the mood-edge + marker commit). One narrow exception is the existing `LibrarySongsDb.add_songs_to_library_batch` intent: its facade-owned batch upsert coordinates the song-row upsert and create-only state initialization with `commit=False` repository calls, then performs one commit (or rollback) for that batch. This exception is not a caller-managed transaction contract. Tier 3 intent facades under `api/` otherwise remain thin semantic delegators: they validate/canonicalize immutable domain commands and forward one intent, but they do not import/own table SQL, resolve storage IDs, call `commit()`/`rollback()`/`remove()`, or own retry/disposal.

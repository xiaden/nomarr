...
class LibraryTagsDb:
    ...
    def _resolve_song_ids_map(self, songs: Sequence[SongIdentity]) -> dict[SongIdentity, int]:
        """Resolve a batch of song locators to storage ids, keyed by identity.

        Set-based: libraries are resolved by ``library_uuid`` in one query and
        songs in one query — no per-song/per-library SQL loop in the facade.
        """
        if not songs:
            return {}

...
## Key Modules
 ...
 | `move_detection_comp` | Per-file move detection: a bounded, library-scoped, persistence-backed chromaprint lookup with 1 s duration tolerance; a match is a relocation only when its source is genuinely absent from disk, and only when the candidate population was observed in full |
...
- **Move detection:** Each newly discovered file is resolved individually via a bounded, library-scoped, persistence-backed chromaprint lookup — the DB listing is the primary and only lookup, never a whole-library in-memory comparison. The lookup fetches one sentinel row beyond the nominal bound and reports the population as incomplete when it exists, so a deterministic first window is never mistaken for the whole candidate set. Candidates are filtered by the 1 s duration tolerance. The chromaprint column has no existence filter, so a match is a real relocation only when its source is genuinely absent from disk and the population is complete — a live duplicate must not be repointed, and a truncated window must never drive automatic relocation.

...
def get_library_by_name(
    ...
    *,
    ...
    doc = db.get_library_by_name(name)
    if doc is None or not include_scan:
        return doc
    return _merge_scan_state(db, doc)
...
def list_library_records(
    ...
    *,
    ...
    docs = db.list_libraries(enabled_only=enabled_only)
...
def update_library_record(
    ...
    **fields: Any,
    ...
    db.update_library(normalize_library_id(library_id), update_fields)
...
def find_ml_complete_libraries(db: Database, min_files: int) -> list[dict[str, Any]]:
    ...
    state_docs = cast(
        ...
        db.library_pipeline_states.get(
            pipeline_state=PIPELINE_ML_RUNNING,

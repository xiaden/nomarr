#!/usr/bin/env python3
# ======================================================================
#  Essentia Autotag - Heads (decision logic)
#  Interprets head outputs into tags using cascades/thresholds.
#  NOTE: This module is deliberately inference-agnostic: it does NOT
#        load or execute TF graphs. It only turns outputs → decisions.
# ======================================================================
...
from nomarr.components.ml.models.discovery import Sidecar
...
class Cascade:
    ...
    high: float = 0.8  # strict tier
    medium: float = 0.75  # regular tier
    low: float = 0.6  # loose tier
    # Relative ratio thresholds: require p / counter_p >= ratio_*
    ratio_high: float = 1.20
    ratio_medium: float = 1.10
    ratio_low: float = 1.02
    # Absolute gap thresholds: require p - counter_p >= gap_*
    gap_high: float = 0.15
    ...
    def from_sidecar(cls, sc: Sidecar, fallback: Cascade | None = None) -> Cascade:
        ...
        if isinstance(levels, dict):
            return cls(
                ...
                gap_medium=float(levels.get("gap_medium", getattr(fallback, "gap_medium", 0.10) if fallback else 0.10)),
...
class HeadSpec:
    name: str
    kind: str  # "regression" | "multilabel" | "multiclass"
    labels: list[str] = field(default_factory=list)
    cascade: Cascade = field(default_factory=Cascade)
    # Per-label thresholds (multilabel); if empty, fall back to cascade.low
    label_thresholds: dict[str, float] = field(default_factory=dict)
    # For multiclass adaptive selection
    min_conf: float = 0.15  # minimum probability for any class to be emitted
    max_classes: int = 5  # hard cap to avoid runaway outputs
    top_ratio: float = 0.5  # include classes >= top_p * top_ratio
    prob_input: bool = True  # True if upstream already produced probabilities

    @classmethod
    def from_sidecar(cls, sc: Sidecar) -> HeadSpec:
        meta = sc.data or {}
        # Ensure string defaults for required fields
...
def decide_multiclass_adaptive(scores: np.ndarray, spec: HeadSpec) -> dict[str, Any]:
    ...
    if not spec.prob_input:
        # For multiclass heads that delivered logits, normalize across dim
        probs = _normalize(probs)

    order = np.argsort(-probs)
    out: dict[str, Any] = {}
    emitted = 0
    # top probability for ratio checks
    if len(order) == 0:
        return out
...
class HeadDecision:
    ...
    def to_head_outputs(
        ...
        """
        from nomarr.components.ml.models.discovery import HeadOutput  # Late import to avoid circular dependency
    ...
    *,
    ...
    if kind == "regression":
        ...
        if emit_all_scores:
            ...
            # Normalize for multiclass to a probability simplex
            if not spec.prob_input:
                vmax = np.max(probs)

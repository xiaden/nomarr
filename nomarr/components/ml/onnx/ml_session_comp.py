...
def create_session(
    ...
    CUDA is unavailable). Always appends CPUExecutionProvider as the final
    fallback. When *vram_limit_bytes* is provided for a GPU session, it is
    applied as ``gpu_mem_limit`` in the CUDA provider options.

...
async def update_model_vram_from_oom(db: Database, model_path: str, requested_bytes: int) -> int:
    """Write a corrected VRAM limit after a BFC arena OOM."""
    raw_doc = cast("dict[str, Any] | None", await db.app.get_config_option(f"{_META_PREFIX}{model_path}"))
    ...
    await db.app.update_config_option(f"{_META_PREFIX}{model_path}", {"value": str(new_limit)})
    logger.warning(

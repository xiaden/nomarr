...
def set_song_tags(db: Database, song: SongIdentity, name: str, values: list[TagValue]) -> None:
    """Replace all tags for one natural song locator and name pair."""
...
def set_song_tags_batch(db: Database, entries: list[dict[str, Any]]) -> None:
    """Replace tags for many natural song locators and name pairs."""
    if not entries:
        return

    replacements_by_song: dict[SongIdentity, dict[str, list[TagValue]]] = {}
    for entry in entries:
        song = entry["song"]
        if not isinstance(song, SongIdentity):
            raise TypeError("song entry must be a SongIdentity")
        name = str(entry["name"])
        values = [_validate_tag_value(value) for value in entry["values"]]
        song_replacements = replacements_by_song.setdefault(song, {})
    ...
    for song, replacements_by_name in replacements_by_song.items():
        db.library.replace_song_tags(
            song,
            _merge_replaced_assignments(
                existing_tags_by_song.get(song, ()),

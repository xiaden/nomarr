...
def add_regression_mood_tiers(regression_heads: list[tuple[Any, list[float]]], framework_version: str) -> list[Any]:
    ...
    for head_info, segment_values in regression_heads:
        ...
        if is_high:
            ...
            model_key_high, calib_id_high = head_info.build_versioned_tag_key(high_term, framework_version=framework_version, calib_method="none", calib_version=0)
            model_key_low, calib_id_low = head_info.build_versioned_tag_key(low_term, framework_version=framework_version, calib_method="none", calib_version=0)
            outputs.append(HeadOutput(head=head_info, model_key=model_key_high, label=high_term, value=mean_val, tier=None, calibration_id=calib_id_high))
            outputs.append(HeadOutput(head=head_info, model_key=model_key_low, label=low_term, value=1.0 - mean_val, tier=None, calibration_id=calib_id_low))
            logger.debug(f"[aggregation] Regression neutral: {head_name} → both {high_term}/{low_term} (mean={mean_val:.3f}, std={std_val:.3f})")
            continue
        model_key, calibration_id = head_info.build_versioned_tag_key(mood_term, framework_version=framework_version, calib_method="none", calib_version=0)
        tier: str | None = None
        if std_val >= _ACCEPTABLE:
            logger.debug(f"[aggregation] Regression no tier: {head_name} → {mood_term} (mean={mean_val:.3f}, std={std_val:.3f} - high variance)")
        else:
            ...
            if std_val < _VERY_STABLE and intensity >= 0.8:
                ...
                tier = "low"
            logger.debug(f"[aggregation] Regression mood: {head_name} → {mood_term} (mean={mean_val:.3f}, std={std_val:.3f}, intensity={intensity:.2f}, tier={tier})")
        outputs.append(HeadOutput(head=head_info, model_key=model_key, label=mood_term, value=mean_val, tier=tier, calibration_id=calibration_id))
    return outputs
LABEL_PAIRS = [("happy", "sad", "peppy", "sombre"), ("aggressive", "relaxed", "aggressive", "relaxed"), ("electronic", "acoustic", "synth-like", "acoustic-like"), ("party", "not_party", "party-like", "not party-like"), ("danceable", "not_danceable", "easy to dance to", "hard to dance to"), ("bright", "dark", "bright timbre", "dark timbre"), ("male", "female", "low-pitch vocal", "high-pitch vocal"), ("tonal", "atonal", "tonal", "atonal"), ("instrumental", "voice", "instrumental only", "has vocals")]
_STRONG_THRESHOLD = 0.7
_WEAK_THRESHOLD = 0.3
_VERY_STABLE = 0.08
_STABLE = 0.15
_ACCEPTABLE = 0.25
_MOOD_MAPPING = {"approachability_regression": ("mainstream", "fringe"), "engagement_regression": ("engaging", "mellow")}

def _build_tier_map(head_outputs: list[Any], calibrations: dict[str, dict[str, Any]] | None) -> dict[str, tuple[str, float, str]]:
...
def _compute_suppressed_keys(tier_map: dict[str, tuple[str, float, str]], label_pairs: list[tuple[str, str, str, str]]) -> set[str]:
    ...
    for pos_pat, neg_pat, _pos_label, _neg_label in label_pairs:
        pos_keys = [k for k in tier_map if pos_pat in k.lower() and f"non_{pos_pat}" not in k.lower() and (f"not_{pos_pat}" not in k.lower())]
        neg_keys = [k for k in tier_map if neg_pat in k.lower() and f"non_{neg_pat}" not in k.lower() and (f"not_{neg_pat}" not in k.lower())]
        logger.debug(f"[aggregation] Checking pair ({pos_pat}, {neg_pat}): found pos={len(pos_keys)} neg={len(neg_keys)}")
...
def _build_label_map(tier_map: dict[str, tuple[str, float, str]], label_pairs: list[tuple[str, str, str, str]]) -> dict[str, str]:
...
def _build_tier_term_sets(tier_map: dict[str, tuple[str, float, str]], suppressed_keys: set[str], label_map: dict[str, str]) -> BuildTierTermSetsResult:
    ...
    logger.debug(f"[aggregation] Mood aggregation: strict={len(strict_terms)}, regular={len(regular_terms)}, loose={len(loose_terms)}")
...
def aggregate_mood_tiers(head_outputs: list[Any], calibrations: dict[str, dict[str, Any]] | None=None) -> dict[str, Any]:
    ...
    """
    logger.debug(f"[aggregation] aggregate_mood_tiers called with {len(head_outputs)} HeadOutput objects, calibrations={calibrations is not None}")

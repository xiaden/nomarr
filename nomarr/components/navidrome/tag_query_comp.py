...
def get_nomarr_tag_names(db: Database) -> list[str]:
    """Return all unique tag names used by Nomarr."""
...
def find_files_matching_tag(
    ...
    """Return file IDs matching a tag condition."""
...
def get_short_to_versioned_mapping(
    ...
    """Return mapping from short names to versioned storage keys.

    Most short names map to exactly one versioned key; future calibrations

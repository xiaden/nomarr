...
class SubsonicClient:
    """Synchronous HTTP client for the Subsonic/Navidrome API using token-based auth."""
    ...
    def get_album_list2(self, type: str, size: int, offset: int) -> list[dict[str, Any]]:
        ...
        resp = self._request("getAlbumList2.view", {"type": type, "size": str(size), "offset": str(offset)})
        album_list = resp.get("albumList2", {})
        albums: list[dict[str, Any]] = album_list.get("album", [])
        return albums

    def get_album(self, album_id: str) -> dict[str, Any]:
        """Get album details including songs via ``getAlbum``."""

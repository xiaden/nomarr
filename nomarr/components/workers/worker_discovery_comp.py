...
def discover_next_file(
    ...
    """Discover next untagged file, excluding too_short and claimed files."""
...
def claim_file(db: Database, file_id: str, worker_id: str) -> bool:
    """Claim file for processing via deterministic _key; returns False if dup."""
    payload = {
...
def try_insert_or_steal_claim(
    ...
    """Insert claim or steal expired one; returns True if acquired."""
    try:
        db.app.add_claim(payload)
    except DuplicateKeyError:
...
def cleanup_stale_claims(db: Database, heartbeat_timeout_ms: int) -> int:
    """Remove stale claims from inactive workers and ineligible files."""
...
def discover_and_claim_file(
    ...
    """Discover and claim next file; returns None on conflict for retry."""

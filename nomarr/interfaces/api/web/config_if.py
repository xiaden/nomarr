...
@router.get("")
def get_config(
    _session: Annotated[dict, Depends(verify_session)], config_service: Annotated[Any, Depends(get_config_service)]
) -> ConfigResponse:
    ...
    try:
        ...
        filtered_result = WebConfigResult(
            config=filtered_config, internal_info=result.internal_info, worker_enabled=result.worker_enabled
        )
...
def update_config(
    ...
    try:
        ...
        return ConfigUpdateResponse(
            status="success", message=f"Config '{key}' updated. Use 'Restart Server' for changes to take full effect."
        )

...
class MatchResultResponse(BaseModel):
    ...
    input_track: PlaylistTrackInputResponse = Field(..., description="Original track from streaming playlist")
    status: Literal["exact_isrc", "exact_metadata", "fuzzy", "ambiguous", "not_found"] = Field(
        ..., description="Type of match achieved"
    )
    confidence: float = Field(..., ge=0.0, le=1.0, description="Match confidence score (0-1)")
    matched_file: MatchedFileInfoResponse | None = Field(None, description="Matched library file with metadata")
    ...
    def from_dto(cls, dto: MatchResult) -> MatchResultResponse:
        ...
        return cls(
            ...
            matched_file=(MatchedFileInfoResponse.from_dto(dto.matched_file) if dto.matched_file else None),
            alternatives=[MatchedFileInfoResponse.from_dto(alt) for alt in dto.alternatives],
        )
...
class PlaylistMetadataResponse(BaseModel):
    ...
    source_platform: Literal["spotify", "deezer"] = Field(..., description="Source streaming platform")
...
class ConvertPlaylistResponse(BaseModel):
    ...
    playlist_metadata: PlaylistMetadataResponse = Field(..., description="Source playlist metadata")
    m3u_content: str = Field(..., description="Generated M3U playlist content (ready to save)")
    total_tracks: int = Field(..., description="Total tracks in source playlist")
    matched_count: int = Field(..., description="Successfully matched tracks (exact + fuzzy)")
    exact_matches: int = Field(..., description="Tracks matched by ISRC or exact metadata")
    fuzzy_matches: int = Field(..., description="Tracks matched via fuzzy matching")
    ambiguous_count: int = Field(..., description="Tracks with multiple possible matches (need review)")
    not_found_count: int = Field(..., description="Tracks that could not be matched")
    match_rate: float = Field(..., ge=0.0, le=1.0, description="Percentage matched (0-1)")
    ...
    def from_dto(cls, dto: PlaylistConversionResult) -> ConvertPlaylistResponse:
        """Convert DTO to response model."""
        return cls(
            playlist_metadata=PlaylistMetadataResponse.from_dto(dto.playlist_metadata),
            ...
            unmatched_tracks=[PlaylistTrackInputResponse.from_dto(r.input_track) for r in dto.get_unmatched()],
            ambiguous_matches=[MatchResultResponse.from_dto(r) for r in dto.get_ambiguous()],
            all_matches=[MatchResultResponse.from_dto(r) for r in dto.match_results],
        )
...
class SpotifyCredentialsStatusResponse(BaseModel):
    ...
    configured: bool = Field(..., description="True if Spotify credentials are configured")

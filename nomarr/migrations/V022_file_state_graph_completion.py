...
def upgrade(db: DatabaseLike) -> None:
    ...
    repoint_cursor = db.aql.execute(  # type: ignore[union-attr]
        """
        LET old_edges = (
        ...
        FOR e IN old_edges
            ...
            INSERT { _from: e._from, _to: "file_states/tagged" }
            INTO file_has_state OPTIONS { ignoreErrors: true }
            RETURN 1
        """
    )
    repoint_results = list(repoint_cursor)  # type: ignore[arg-type]
    repoint_count = len(repoint_results)
    logger.info(f"[V022] Repointed {repoint_count} ml_tagged → tagged edges")

    # Split reconciled → tags_written + tags_current
    split_cursor = db.aql.execute(  # type: ignore[union-attr]
        """
        LET old_edges = (
            FOR e IN file_has_state
        ...
        FOR e IN old_edges
            REMOVE e IN file_has_state
            INSERT { _from: e._from, _to: "file_states/tags_written" }
            INTO file_has_state OPTIONS { ignoreErrors: true }
            INSERT { _from: e._from, _to: "file_states/tags_current" }
            INTO file_has_state OPTIONS { ignoreErrors: true }
            RETURN 1
        """
    )
    split_results = list(split_cursor)  # type: ignore[arg-type]
    ...
    for positive, negative in state_axes:
        ...
        seed_cursor = db.aql.execute(  # type: ignore[union-attr]
            """
            FOR f IN library_files
                ...
                INSERT { _from: f._id, _to: @negative }
                INTO file_has_state OPTIONS { ignoreErrors: true }
                RETURN 1
            """,
            bind_vars={"positive": positive_id, "negative": negative_id},
        )
        seed_results = list(seed_cursor)  # type: ignore[arg-type]

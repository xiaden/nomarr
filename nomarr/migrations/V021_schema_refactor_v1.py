...
def upgrade(db: DatabaseLike) -> None:
    ...
    # P4-S1: Rewrite meta documents — copy key property to _key, drop key property
    # First: docs where _key != key need UPSERT to create new doc with correct _key
    if db.has_collection("meta"):
        db.aql.execute(  # type: ignore[union-attr]
            """
            FOR doc IN meta
                FILTER doc.key != null AND doc._key != doc.key
                LET newDoc = UNSET(MERGE(doc, { _key: doc.key }), "key")
                UPSERT { _key: doc.key }
                INSERT newDoc
                UPDATE UNSET(doc, "key") IN meta
            """
        )
        # Second: handle docs where _key already matches key but key property still exists
        db.aql.execute(  # type: ignore[union-attr]
            """
            FOR doc IN meta
                FILTER doc.key != null
                UPDATE doc WITH { key: null } IN meta OPTIONS { keepNull: false }
            """
        )
        logger.info("[V021] Migrated meta documents: key property → _key")
    ...
    # P5-S3/S4/S5: Drop obsolete indexes
    for coll_name, fields in [
        ("library_files", ["library_id"]),
        ("library_files", ["library_id", "path"]),
        ("library_folders", ["library_id"]),
    ]:
        if db.has_collection(coll_name):
            coll = db.collection(coll_name)  # type: ignore[union-attr]
            for idx in coll.indexes():  # type: ignore[union-attr]
                if idx.get("fields") == fields and idx.get("type") == "persistent":
                    coll.delete_index(idx["id"])  # type: ignore[union-attr]
                    logger.info(f"[V021] Dropped index {fields} from {coll_name}")
                    break
